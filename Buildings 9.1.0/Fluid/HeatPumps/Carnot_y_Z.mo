...
model Carnot_y_Z
  ...
  extends Buildings.Fluid.Chillers.BaseClasses.PartialCarnot_y_Z;

initial equation
  assert(COP_nominal_ConGro > 1, "The nominal COP of a heat pump ConGro must be bigger than one.");
  assert(COP_nominal_GroEva > 1, "The nominal COP of a heat pump GroEva must be bigger than one.");
equation
  connect(port_b3, port_b3)
    annotation (Line(points={{-96,-66},{-96,-66}}, color={0,127,255}));
  annotation (
...
First implementation.
    ...
    Diagram(coordinateSystem(extent={{-120,-100},{160,140}})),
    Icon(coordinateSystem(extent={{-120,-100},{160,140}})));
end Carnot_y_Z;
